...
describe("Sidebar wanted submenu selection", () => {
  ...
  test("desktop highlights Movies sub on /wanted/movies", () => {
    ...
    const moviesLink = moviesLinks.find((l) => l.getAttribute("href") === "/wanted/movies");
    expect(moviesLink).toBeDefined();
    // The selected item should render as bold
    expect(moviesLink.style.fontWeight === "bold" || moviesLink.style.fontWeight === "700").toBe(true);
  });
  ...
  test("mobile highlights Series sub on /wanted/series", () => {
    ...
    const seriesLink = seriesLinks.find((l) => l.getAttribute("href") === "/wanted/series");
    expect(seriesLink).toBeDefined();
    expect(seriesLink.style.fontWeight === "bold" || seriesLink.style.fontWeight === "700").toBe(true);
  });
});
