...
export default function ExtrasSettings() {
  ...
  useEffect(() => {
    Promise.all([axios.get("/api/tmdb/extratypes"), axios.get("/api/settings/extratypes"), axios.get("/api/settings/canonicalizeextratype"), axios.get("/api/settings/ytdlpflags")])
      .then(([tmdbRes, plexRes, mapRes, ytRes]) => {
        setTmdbTypes(tmdbRes.data.tmdbExtraTypes);
        if (!Array.isArray(plexRes.data)) { setError("Server response missing mapp array in /api/settings/extratypes"); return; }
        const mapp = plexRes.data; setPlexTypes(mapp);
        const settingsFromMapp = {};
        for (const entry of mapp) { if (entry?.key) settingsFromMapp[entry.key] = !!entry.value; }
        const initialMapping = { ...mapRes.data.mapping };
        for (const type of tmdbRes.data.tmdbExtraTypes) { if (!initialMapping[type]) initialMapping[type] = "Other"; }
  ...
  const handleSave = async () => {
    setSaving(true); setToast(""); setError("");
    try { await axios.post("/api/settings/extratypes", settings); await axios.post("/api/settings/canonicalizeextratype", { mapping }); await axios.post("/api/settings/ytdlpflags", ytFlags); setToast("All settings saved successfully!"); setToastSuccess(true); } catch { setError("Failed to save one or more settings"); setToast("Failed to save one or more settings"); setToastSuccess(false); } finally { setSaving(false); }
  };
  ...
  return (
    <Container>
      <ActionLane buttons={[{ icon: saving ? <FontAwesomeIcon icon={faSpinner} spin /> : <FontAwesomeIcon icon={faSave} />, label: "Save", onClick: handleSave, disabled: saving || !isChanged, loading: saving, showLabel: globalThis.window === undefined ? true : globalThis.window.innerWidth > 900 }]} error={error} />
