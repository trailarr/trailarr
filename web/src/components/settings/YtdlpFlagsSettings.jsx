...
export default function YtdlpFlagsSettings() {
  ...
  useEffect(() => { axios.get("/api/settings/ytdlpflags").then((res) => setYtFlags(res.data)).catch(() => setError("Failed to load yt-dlp flags")); }, []);
  const handleYtFlagChange = (key, value) => setYtFlags((p) => ({ ...p, [key]: value }));
  const handleSave = async () => {
    if (typeof ytFlags.maxSleepInterval === "number" && typeof ytFlags.sleepInterval === "number" && ytFlags.maxSleepInterval < ytFlags.sleepInterval) { setError("Max Sleep Interval must not be lower than Sleep Interval."); return; }
    setSaving(true); setError(""); setToast("");
    try { await axios.post("/api/settings/ytdlpflags", ytFlags); setToast("yt-dlp flags saved"); setToastSuccess(true); } catch { setError("Failed to save yt-dlp flags"); setToast("Failed to save yt-dlp flags"); setToastSuccess(false); } finally { setSaving(false); }
  };
  return (
    <Container>
      <ActionLane buttons={[{ icon: saving ? <FontAwesomeIcon icon={faSpinner} spin /> : <FontAwesomeIcon icon={faSave} />, label: "Save", onClick: handleSave, disabled: saving, loading: saving, showLabel: globalThis.window === undefined ? true : globalThis.window.innerWidth > 900 }]} error={error} />
          ...
          {YTDLP_FLAGS.map(({ key, label, type }) => {
            ...
            return (
              <div key={key} style={{ display: "flex", alignItems: "center", marginBottom: 16 }}>
                {type === "boolean" ? (
                  <>
                    <input type="checkbox" id={key} checked={!!ytFlags[key]} onChange={() => handleYtFlagChange(key, !ytFlags[key])} disabled={disabledDueToWriteSubs} style={{ marginRight: 12, accentColor: isDark ? "#2563eb" : "#6d28d9" }} />
                  ...
                  <>
                    ...
                    <input type={type === "number" ? "number" : "text"} id={key} value={ytFlags[key] ?? ""} onChange={(e) => handleYtFlagChange(key, type === "number" ? Number(e.target.value) : e.target.value)} disabled={disabledDueToWriteSubs} style={{ marginLeft: 12, width: 120, minWidth: 80, maxWidth: 160, padding: "0.15em 0.5em", fontSize: 13, border: "1px solid", borderColor: isDark ? "#444" : "#ccc", borderRadius: 4, background: isDark ? "#23232a" : "#fff", color: isDark ? "#e5e7eb" : "#222" }} />
                  </>
                )}
              </div>
            );
